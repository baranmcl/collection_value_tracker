...
- **Prevention.** Start every write session with `git worktree add .claude/worktrees/<slug> -b <branch-name>`. The worktree's working tree is insulated from resets or pulls that target the root checkout. The "untracked files survive" quirk of `git reset --hard` is an acceleration of its scope, not a design to rely on — worktrees provide actual isolation.
